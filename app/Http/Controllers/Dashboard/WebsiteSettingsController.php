...
class WebsiteSettingsController extends Controller implements HasMiddleware
{
    ...
    public function update(Request $request, WebsiteSetting $websiteSetting)
    {
        ...
        if($request->hasFile('cover')) {
            if($websiteSetting->cover && Storage::disk('public')->exists($websiteSetting->cover)) {
                Storage::disk('public')->delete($websiteSetting->cover);
            }
            $image = $request->file('cover');
            $imagePath = 'website-settings/cover/' . uniqid() . '.webp';
